...
run_with_ius_test() {
    ...
    irun -f ${PROJECT_DIR}/examples/common/scripts/options_ius.f -svseed ${seed} +UVM_TESTNAME=${test} +UVM_VERBOSITY=${verbosity} +UVM_MAX_QUIT_COUNT=${quit_cnt} ${EXTRA_OPTIONS}
}
...
run_with_vcs_test() {
    ...
    vcs -ntb_opts uvm -f ${PROJECT_DIR}/examples/common/scripts/options_vcs.f +ntb_random_seed=${seed} +UVM_TESTNAME=${test} +UVM_VERBOSITY=${verbosity} +UVM_MAX_QUIT_COUNT=${quit_cnt} -R ${EXTRA_OPTIONS}
