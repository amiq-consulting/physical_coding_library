...
export TOP_FILE_NAME=${TOP_MODULE_NAME}.svh
export TOP_FILE_PATH=${EXAMPLE_DIR}/ve/${TOP_FILE_NAME}
...
help() {
    ...
    echo "  -verbosity {UVM_NONE|UVM_LOW|UVM_MEDIUM|UVM_HIGH|UVM_FULL|UVM_DEBUG }]  --> specify the verbosity of a message (default: ${default_verbosity})"
...
run_with_ius_test() {
    ...
    irun -f ${PROJECT_DIR}/examples/common/scripts/options_ius.f -svseed ${seed} +UVM_TESTNAME=${test} +UVM_VERBOSITY=${verbosity} ${EXTRA_OPTIONS}
}
...
run_with_vcs_test() {
    ...
    vcs -ntb_opts uvm -f ${PROJECT_DIR}/examples/common/scripts/options_vcs.f +ntb_random_seed=${seed} +UVM_TESTNAME=${test} +UVM_VERBOSITY=${verbosity} -R ${EXTRA_OPTIONS}
...
run_with_questa_test() {
    ...
    vsim -${ARCH_BITS} -novopt ${TOP_MODULE_NAME} -sv_seed ${seed} +UVM_TESTNAME=${test} +UVM_VERBOSITY=${verbosity} ${EXTRA_OPTIONS}
}
